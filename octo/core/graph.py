...
def _build_supervisor_prompt(
    ...
    if not engine_mode:
        ...
        parts.append(
            ...
            "Two modes:\n"
            "- **agent** (preferred for most tasks): In-process LangGraph agent with full access "
            "to ALL tools including MCP tools (Sirens TTS/STT/images/video, filesystem, etc.). "
            "Can call `task_complete` when done or `escalate_question` to ask the user.\n"
            "- **process**: Subprocess (shell commands, `claude -p`). Has NO MCP tool access. "
            "Use only for Claude Code tasks or shell commands.\n\n"
            "**Choosing the right mode:**\n"
            "- Media generation (TTS, STT, video, images) → **agent** (needs MCP tools)\n"
            "- Workflows, analysis using MCP servers → **agent**\n"
            "- `claude -p` commands → **process**\n"
            ...
            "ALWAYS dispatch these as background **agent** tasks so the user isn't blocked."
        )
